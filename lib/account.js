...
export const isMuted = (actorId) => {
  const muted = getMuted();
  return muted.includes(actorId);
}
...
export const writeMuted = (muted) => {
  return writeJSONDictionary(mutedFile, muted)
}
...
export const getMuted = () => {
  return readJSONDictionary(mutedFile).map((f) => {
    if (typeof f === 'string') {
      // map old format to new format just in case
      // TODO: remove this before release
      return {
...
export const mute = async (request) => {
  logger('muting someone');
  const {
    actor
  } = await fetchUser(request.object.object);
  if (!actor) {
